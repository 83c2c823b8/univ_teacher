...
\documentclass[b4papaer,landscape,twocolumn]{ltjsarticle}
...
\twocolumn[{\Large{4章\ 関数\ \(y=ax^2\)} }\\ \Large{\S 1 関数とグラフ}\\]
\section*{１次関数の復習}
\(y= -x + 1\)のグラフを書いてみよう．
\begin{center}
...
\begin{center}
  ...
  \begin{tikzpicture}
    \draw[dotted,thin,step=0.5] (-3,-0.5) grid (3,5.5);
    \draw [->,thick] (-3.5,0) -- (3.5,0) node [right]{\(x\)};
    \draw [->,thick] (0,-1) -- (0,6.0) node [above]{\(y\)};
    \node [below left] (0,0){O};
...
点を取っていないところを確認するため，原点近くのグラフの様子を調べよう．
\begin{center}
